...
def build(root: Path, allow_dirty: bool, allow_warnings: bool, keep_failed: bool = False) -> Path:
    ...
    try:
        ...
        run([kicad, "pcb", "export", "pdf", "--output", str(temp / "assembly/assembly-front.pdf"), "--layers", "F.Fab,F.Silkscreen,Dwgs.User,Cmts.User,Edge.Cuts", "--include-border-title", "--hide-DNP-footprints-on-fab-layers", "--sketch-pads-on-fab-layers", "--black-and-white", "--mode-single", "--check-zones", str(pcb)], root)
        run([kicad, "pcb", "export", "pdf", "--output", str(temp / "assembly/assembly-back.pdf"), "--layers", "B.Fab,B.Silkscreen,Dwgs.User,Cmts.User,Edge.Cuts", "--include-border-title", "--mirror", "--hide-DNP-footprints-on-fab-layers", "--sketch-pads-on-fab-layers", "--black-and-white", "--mode-single", "--check-zones", str(pcb)], root)
